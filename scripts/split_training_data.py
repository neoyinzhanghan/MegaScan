...
def split_data(data_dir, save_dir, train_ratio, val_ratio, test_ratio):
    ...
    for class_folder in tqdm(class_folders, desc="Processing classes"):
        ...
        for image in tqdm(
            train_images, desc=f"Copying train images for {class_folder}", leave=False
        ):
        ...
        for image in tqdm(
            val_images, desc=f"Copying val images for {class_folder}", leave=False
        ):
        ...
        for image in tqdm(
            test_images, desc=f"Copying test images for {class_folder}", leave=False
        ):
